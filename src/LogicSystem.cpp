	...
	for (;;) {
		...
		if (_b_stop) {
			while (!_msg_que.empty()) {
				...
				spdlog::info("LogicSystem正在处理消息，消息id为%d",
...
void LogicSystem::RegisterCallBacks() {
	...
	_fun_callbacks[MSG_LOGIN] =
		std::bind(&LogicSystem::handleUserLogin, this, placeholders::_1,
				  placeholders::_2, placeholders::_3);
}
...
void LogicSystem::HelloWordCallBack(shared_ptr<CSession> session,
	...
	spdlog::info("接收到消息id:{}，内容为:{}", id, data.c_str());
	// Json::Reader reader;
	// Json::Value root;
	...
	// root["data"].asString(); std::string return_str = root.toStyledString();
...
void LogicSystem::handleUserLogin(shared_ptr<CSession> session,
	...
	spdlog::info("接收到消息id:{}，内容为:{}", msg_id, msg_data.c_str());
